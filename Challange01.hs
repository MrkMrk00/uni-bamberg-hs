...
_indexOf el lst = 
    let withI idx el (head:tail)
            | head == el = idx
            | null tail = -1
            | otherwise = withI (idx + 1) el tail in
    withI 0 el lst
...
findCharString _char lst =
    let char = _charToLower _char in 
        filter (\ e -> char `elem` _toLower e) lst
...
sortStrings inputList = 
    let lowerCaseList = map _toLower inputList in 
        map (\ e -> inputList !! _indexOf e lowerCaseList) (doInsert [] lowerCaseList)
    where doInsert dest (head:rest)
...
_getCharScore char = 
    let score = _indexOf (_charToLower char) ['a'..'z'] in
    if score == -1 then 0 else score
